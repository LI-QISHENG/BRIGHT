...
def main():
    ...
    if hd:
        ...
        # Exclude any "Drug resistance" in PathwayL2 (case-insensitive, NaN-safe)
        mask_dr = df_out['PathwayL2'].str.contains('drug resistance', case=False, na=False)
        df_out = df_out[~mask_dr].copy()

    df_out.to_csv(out_csv, index=False)
    print(f"Wrote {out_csv} with {len(df_out)} KO records{' (Human Diseases only, excluding Drug resistance)' if hd else ''}.")
